...
# Your groups mapping remains unchanged
groups = {
...
def submit_result(request):
    if request.method == 'POST':
        ...
        student.score = dominant_trait  # Save the dominant trait as the student's score
        student.save()
        ...
        # --- Sending first email with personality details ---
        template = EMAIL_TEMPLATES.get(dominant_trait.lower()) if dominant_trait else None
        if template and student.email:
            send_mail(
                template['subject'],
                template['body'],
                'fahd.ayman841@gmail.com',
                [student.email],
                fail_silently=False,
            )
        
        # --- Initially assign student to a group based on capacity ---
        # (This assignment will be rebalanced once all students have taken the test.)
        ...
        if not assigned_group:
            # Fallback: if somehow all groups are full, assign to the one with the fewest members.
            smallest_group = min(available_groups, key=lambda g: g.students.count(), default=None)
            if smallest_group:
                smallest_group.students.add(student)
        
        # --- Rebalance groups when all students have passed the test ---
        ...
        if all_students.filter(score__isnull=False).count() == classroom.students_count:
            ...
            # Now you can send the group email (if needed) using the rebalanced groups.
            for s in all_students:
                # Get the group the student now belongs to
                student_groups = s.groups.all()
                if student_groups.exists():
                    group = student_groups.first()  # assuming one group per student
                    # Build list of other group members (exclude current student)
                    group_members = group.students.exclude(id=s.id)
                    if group_members.exists():
                        members_info = []
                        for member in group_members:
                            # e.g. "Yassine - Persister"
                            info = f"{member.first_name} - {member.score}"
